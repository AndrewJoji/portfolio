...
### Blog
A `/blog` page with two parts: **Posts**, for things I find interesting, and a **Bookshelf** of what I'm reading and have finished. Posts live in `src/lib/blog.ts` and use the same story sections as the role and project pages, so a post can mix text with photos, galleries, YouTube videos, quotes and links.

The bookshelf shows each book's cover with a category tag underneath, and clicking a book opens its own page with my takeaway and links. Covers load from Open Library by ISBN. Dropping an image at `public/blog/books/<slug>.jpg` uses that image instead, and if neither exists, a title-and-author cover is drawn instead. A book can link to a full post with `postSlug`.
